...
void main() {
    ...
    Generator generator = new Generator("Cockroach World?", new Amount(CURRENCY, 5e18), 1.12f,
            new Amount(CURRENCY, 1e15));
    double generatorProduction = generator.getBaseProduction().amount() * 10001 * 2;
    ...
    for (int l = startValue; l <= startValue + 100; l += 1) {
        ...
        for (int d = startValue + 1; d <= l; d += 1) {
            ...
            double generatorTime = generator.getCost(d - 1).amount() / ((d - 1) * generatorProduction + lucaProduction);
            time += generatorTime;
            System.out.println(
                    "generator" + ": " + d + ", generatorTime: " + generatorTime / 60 + " (time " + time / 60 + ")");
        }
        double improvementTime = improvementCost / (l * generatorProduction + lucaProduction);
